...
LOAD_EPOCH=10
SUBSAMPLE=new
LOG_DIR=analysis_notes/preprocess_robustness
OUTPUT_ROOT=output/preprocess_robustness

PREPROCESSES=(none jpeg90 jpeg70 blur resize colorjitter)
...
find_model_dir() {
    ...
    if [[ "${dataset}" == "caltech101" && "${trainer}" == "BadClip" ]]; then
        echo "[INFO] Candidate directories for manual confirmation:"
        find output -type d | grep -i "caltech101" | grep -i "BadClip" | grep "seed${seed}" || true
    else
        find output -type d | grep -i "${dataset}" | grep -i "${trainer}" | grep "seed${seed}" || true
    fi
