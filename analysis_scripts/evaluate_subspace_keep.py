...
def logits_with_given_image_features(model, image_features_for_logits, image_features_for_prompt):
    ...
    # BadCLIP/CLIP 可能运行在 fp16，所以送进 prompt learner 和 text encoder 前要对齐 dtype。
...
def combine_basis(*bases):
    """
    Concatenate multiple basis matrices and orthonormalize them.
    """
    a = np.concatenate(bases, axis=1).astype("float32")
    q, _ = np.linalg.qr(a)
...
def ablate_delta_by_basis(clean_feat, bd_feat, basis):
    """
    只保留 backdoor shift delta_i 在 basis 上的分量：
      delta_i = f(x+trigger) - f(x)
      delta_i_kept = Proj_basis(delta_i)
      f_kept = normalize(f(x) + delta_i_kept)

    这用于验证该方向/子空间是否足以恢复后门攻击。
    """
...
def main():
    ...
    mean_b = mean_basis(base_delta_i)
    pca1_b = pca_basis(base_delta_i, 1)
    pca3_b = pca_basis(base_delta_i, 3)
    pca5_b = pca_basis(base_delta_i, 5)
    pca10_b = pca_basis(base_delta_i, 10)
    ...
    basis_np = {
        ...
        "keep_pca5": pca5_b,
        "keep_pca10": pca10_b,
        "keep_mean_pca1": combine_basis(mean_b, pca1_b),
        "keep_mean_pca3": combine_basis(mean_b, pca3_b),
        "keep_mean_pca5": combine_basis(mean_b, pca5_b),
        "keep_mean_pca10": combine_basis(mean_b, pca10_b),
        "keep_random1": random_basis(dim, 1, seed=cli_args.seed),
        "keep_random3": random_basis(dim, 3, seed=cli_args.seed),
        "keep_random5": random_basis(dim, 5, seed=cli_args.seed),
        "keep_random10": random_basis(dim, 10, seed=cli_args.seed),
    }
    ...
    lines = []
    lines.append("# Backdoor Subspace Keep-only Evaluation")
    lines.append("")
    ...
    lines.append("## Backdoor ASR when keeping only image-shift subspace")
